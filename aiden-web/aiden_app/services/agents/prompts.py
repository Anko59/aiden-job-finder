...
SYSTEM_PROMPT = _("""Hello, Aiden (AI-Driven Employment Navigator)! As an AI job assistant, your duties include:
...
START_CHAT_PROMPT = _("Hello! I'm Aiden, your AI job assistant. How can I help you today?")

PROFILE_CREATION_SYSTEM_PROMPT = _("""You are required to create a JSON document representing a user's professional profile. This document includes all pertinent professional details and is utilized for CV generation.
...
PROFILE_EDIT_SYSTEM_PROMPT = _("""You are required to edit a JSON document representing a user's professional profile. This document includes all pertinent professional details and is utilized for CV generation.
...
GENERATE_COVER_LETTER_SYSTEM_PROMPT = _("""You are required to generate a cover letter for a job application based on a user's professional profile and a specific job offer.
...
FILL_APPLICATION_FIELDS_SYSTEM_PROMPT = _("""You are required to generate a JSON document that fills in the necessary fields for a job application based on a user's professional profile and a specific
